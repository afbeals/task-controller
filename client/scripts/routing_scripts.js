...
TCommander.config(function($routeProvider, $locationProvider){
	...
	.when('/PassportRegister',{templateUrl: 'partials/passportRegister.html', controller: 'users_controller', caseInsensitiveMatch: true})
	.otherwise({redirectTo: '/Login'});
	$locationProvider.html5Mode({enabled:true});
}).run(['$rootScope', '$location', 'users_service', function ($rootScope, $location, users_service) {
    $rootScope.$on('$routeChangeStart', function (e,next,current) {
    	var path = $location.path();
		//check localstorage for token to see if logged in, if not login or register then return to login
